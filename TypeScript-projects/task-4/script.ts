...
// Define an interface named Person with properties: name, age, and city:
interface Person {
    name: string;
    age: number;
    city: string;
...
searchResult?.addEventListener('click', handleFilter);

// Add an event listener to the input field for the 'keyup' event to check for the Enter key
(document.getElementById('cityName') as HTMLInputElement).addEventListener('keyup', (event) => {
